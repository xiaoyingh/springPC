...
<c:forEach items="${type }" var="bs">
		<li>
			...
			<ul><!--  2-->
				...
				<li>
					<h2>
						<span>+</span><a href="<%=basePath%>backselserieser?id=${bsd.ID }" target="menuFramedd"> ${bsd.typeName }</a>
					</h2>
					<ul>
						<!--  3-->
						...
						<li>
							<h2><a href="<%=basePath%>backselseriessan?id=${asd.ID }" target="menuFramedd">${asd.typeName }</a></h2>
						</li>
						</c:if>
						</c:forEach>
						<!--  3-->
